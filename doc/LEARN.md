...
4. [專案檔案結構](#專案檔案結構)
5. [模組化開發概念](#模組化開發概念)
6. [C++ 標頭檔和實作檔](#cpp-標頭檔和實作檔)
7. [編譯和連結過程](#編譯和連結過程)
8. [函式庫使用](#函式庫使用)
9. [Arduino 框架概念](#arduino-框架概念)
10. [實用指令](#實用指令)
...
GPIO 是可以透過程式控制的接腳，可以設定成輸入或輸出模式。
...
**輸出範例：控制 LED**
```cpp
pinMode(4, OUTPUT);        // 設定 GPIO 4 為輸出
digitalWrite(4, HIGH);     // 輸出高電位（LED 亮）
digitalWrite(4, LOW);      // 輸出低電位（LED 滅）
```

**輸入範例：讀取按鈕**
```cpp
pinMode(13, INPUT);        // 設定 GPIO 13 為輸入
int state = digitalRead(13);  // 讀取按鈕狀態
// state == HIGH → 按鈕按下
// state == LOW  → 按鈕放開
```
...
**ESP32 內部運作電壓是 3.3V**
- CPU、GPIO、記憶體都是 3.3V 邏輯
- 如果 GPIO 直接接 5V 會燒毀晶片 ⚠️

**但 ESP32 同時提供 5V 電源輸出**
- 直接來自 USB 的 5V
- 用於驅動需要較大電流的模組
...
| 元件 | 電源 | 資料信號 | 說明 |
|------|------|----------|------|
| **按鈕模組** | 3.3V 或 5V | 3.3V | ESP32 GPIO 輸出是 3.3V |
| **LED 模組** | 5V | 3.3V | 5V 供電更亮，訊號用 3.3V |
| **WS2812 燈條** | 5V | 3.3V (通常可用) | 電源必須 5V，數據線 3.3V 大多相容 |
| **音頻模組** | 5V | 3.3V | 放大器需要較大電流 |

**重點：ESP32 的 GPIO 輸出/輸入都是 3.3V 邏輯！**
...
**1. 電源軌（+/-）：橫向全部連通**
```
+ 軌：所有孔都連在一起（用於 5V 或 3.3V）
- 軌：所有孔都連在一起（用於 GND）
```

**2. 信號區：垂直連通（同一直列的 5 個孔）**
```
A1-B1-C1-D1-E1 連在一起 ✅
F1-G1-H1-I1-J1 連在一起 ✅
...
```
步驟 1：ESP32 供電
ESP32 的 5V (B1)  → 麵包板左側 + 軌
ESP32 的 GND (B6) → 麵包板左側 - 軌

步驟 2：連接左右電源軌
左側 + 軌 → 用跳線 → 右側 + 軌
左側 - 軌 → 用跳線 → 右側 - 軌

結果：左右兩側都有 5V 和 GND 了！
```
...
**策略：按鈕在 B 側，LED 在 I 側**

```
麵包板配置：
         
B側（左）            I側（右）
[按鈕1] → GPIO 13   GPIO 4 → [LED]
[按鈕2] → GPIO 14   GPIO 5 → [WS2812]
[按鈕3] → GPIO 12   
[按鈕4] → GPIO 33   
[按鈕5] → GPIO 32   
```
...
```
✅ 要加按鈕就往 B 側找空的 GPIO
✅ 要加 LED 就往 I 側找空的 GPIO
```

**4. 避免衝突**
```
✅ 音頻模組橫跨中間（GPIO 25, 26, 22）
✅ 輸入輸出分兩邊，不會搶位置
```
...
1. **先看 PRD 需求**：列出所有需要的輸入/輸出
2. **避開特殊針腳**：GPIO 0, 1, 3, 6-11
3. **分邊配置**：輸入一邊，輸出一邊
4. **預留空間**：為未來擴充保留 GPIO
